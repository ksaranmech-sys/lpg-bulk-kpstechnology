          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {Object.entries(vehicles[0].documentReminders || {}).map(([key, reminder]) => (
              <div key={key} className="list-item" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 0, background: isExpired(reminder.expiryDate) ? '#ffe5e5' : expiresWithin30Days(reminder.expiryDate) ? '#fff4bf' : undefined }}>
                ...
                <span>{reminder.expiryDate ? new Date(reminder.expiryDate).toLocaleDateString('en-IN') : 'Not set'}</span>
                <span style={{ color: reminder.due ? '#b42318' : '#4a5f52' }}>{reminder.status || 'Not set'}</span>
              </div>
            ))}
          </div>
        </div>
