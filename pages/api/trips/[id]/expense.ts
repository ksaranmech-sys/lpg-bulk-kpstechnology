...
// POST /api/trips/:id/expense -> add an "other expense" entry.
// Body: { amount, date, photo: { url, gps }, note? }
export default async function handler(req, res) {
  ...
  if (!parsed.success) return res.status(400).json({ error: parsed.error.issues[0]?.message || "Invalid expense entry" });
  trip.otherExpenses.push(parsed.data);
  await trip.save();
